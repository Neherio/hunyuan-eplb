...
EP_TEST_CONFIG = [
    (TOTAL_CARD_NUM, True, True, DEFAULT_HEIGHT, DEFAULT_WIDTH, True),
]
...
def test_ep(model_name, tmp_path):
    baseline_cache: dict[str, InferenceResult] = {}
    results: list[dict] = []
    device_count = current_omni_platform.get_device_count()
    custom_dir = str(tmp_path / "eplb_output")
    os.makedirs(custom_dir, exist_ok=True)
    os.environ["VLLM_EPLB_OUTPUT_DIR"] = custom_dir
    print(f"\nSetting VLLM_EPLB_OUTPUT_DIR to: {custom_dir}")
    ...
    for tensor_parallel_size, enable_ep, enable_static_eplb, height, width, is_perf_test in EP_TEST_CONFIG:
        ...
        print("\n--- Running the test for initialization of static EPLB. ---")
        _run_inference(
            model_name=model_name,
            tensor_parallel_size=tensor_parallel_size,
            enable_expert_parallel=enable_ep,
        ...
        print("\n--- Running the test for comparison to baseline ---")
        static_eplb_apply_result = _run_inference(
            model_name=model_name,
            tensor_parallel_size=tensor_parallel_size,
            enable_expert_parallel=enable_ep,
            enable_static_eplb=enable_static_eplb,
        ...
        assert len(static_eplb_apply_result.images) == 1
        mean_diff, max_diff, cos_sim_mean, cos_sim_max, mse = _diff_metrics(
            baseline.images[0], static_eplb_apply_result.images[0]
        )
        ...
        result = {
            ...
            "eplb_ms": static_eplb_apply_result.elapsed_ms,
            "mean_diff": mean_diff,
            "max_diff": max_diff,
        ...
        if is_perf_test:
            speedup = (
                baseline.elapsed_ms / static_eplb_apply_result.elapsed_ms
                if static_eplb_apply_result.elapsed_ms > 0
                else 0
            )
            result["speedup"] = speedup
            print(
                f"[eplb_static: {enable_static_eplb}] {tensor_parallel_size} GPUs | "
                f"baseline: {baseline.elapsed_ms:.0f}ms, eplb_static: {static_eplb_apply_result.elapsed_ms:.0f}ms, "
                f"speedup: {speedup:.2f}x"
            )
        else:
            print(
                f"[eplb_static: {enable_static_eplb}] {tensor_parallel_size} GPUs | eplb_static: {static_eplb_apply_result.elapsed_ms:.0f}ms (correctness only)"
            )

        print(
            f"[eplb_static: {enable_static_eplb}] diff: [mean={mean_diff:.6e}, max={max_diff:.6e}], cos_sim: [mean={cos_sim_mean:.6e}, max={cos_sim_max:.6e}], mse: {mse:.6e}"
        )
        assert mean_diff <= DIFF_MEAN_THRESHOLD and max_diff <= DIFF_MAX_THRESHOLD, (
            f"[eplb_static: {enable_static_eplb}] output differs from baseline: mean={mean_diff:.6e}, max={max_diff:.6e}"
        )

    print("\n" + "=" * 90)
    print("SUMMARY")
    print("=" * 90)
    print(f"{'Mode':<15} {'GPUs':<6} {'Size':<10} {'EP Baseline':<12} {'Static EPLB':<12} {'Speedup':<10} {'Status'}")
    print("-" * 90)
    for r in results:
        ...
        print(
            f"{r['prompt']:<15.10} {r['enable_static_eplb']:<6} {r['height']}x{r['width']:<5} "
            f"{baseline_str:<12} {r['eplb_ms']:.0f}ms{'':<7} {speedup_str:<10} {status}"
        )
    print("=" * 90)
